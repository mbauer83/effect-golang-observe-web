...
var eventSchema = schema.Struct[Event]("Event",
	schema.FieldOf("kind", schema.Text(),
		...
		WithDescription("Kind is the runtime's own event vocabulary."),
	...
	schema.FieldOf("attempt", schema.Int64(),
		...
		WithDescription("Attempt is the retry attempt, or zero where the event is not one."),
	schema.FieldOf("atMicros", schema.Int64(),
		...
		WithDescription("AtMicros is the offset from the earliest span in the reading, so an event can be placed on the same timeline the spans are drawn on."),
).WithDescription("Event is one thing that happened inside a span.")
...
var spanSchema = schema.Struct[Span]("Span",
	...
	schema.FieldOf("trace", schema.Text(),
		...
		WithDescription("Trace is the identity of the trace this span belongs to, which the runtime's span counter cannot be: it starts again at one in the next process."),
	schema.FieldOf("depth", schema.Int64(),
		func(value Span) int64 { return value.Depth },
		func(value *Span, field int64) { value.Depth = field }).
		WithDescription("Depth is what nesting carries: the tree flattened, so this shape is not recursive."),
	...
	schema.FieldOf("source", schema.Text(),
		...
		WithDescription("Source is the call site the runtime captured when the span was described."),
	schema.FieldOf("status", schema.Text(),
	...
	schema.FieldOf("startMicros", schema.Int64(),
		...
		WithDescription("StartMicros is the offset from the earliest span in this reading: a duration says how long, and only an offset says when."),
	schema.FieldOf("micros", schema.Int64(),
		...
		WithDescription("Micros is the runtime's own measurement, and zero while the span is open."),
	schema.FieldOf("selfMicros", schema.Int64(),
		...
		WithDescription("SelfMicros is the duration its children did not take, which is what ranking by hotness ranks by."),
	schema.FieldOf("ageMicros", schema.Int64(),
		...
		WithDescription("AgeMicros is how long an open span has been open, which is what a duration cannot say."),
	schema.FieldOf("open", schema.Bool(),
		func(value Span) bool { return value.Open },
		func(value *Span, field bool) { value.Open = field }).
		WithDescription("Open says the span started and has not been seen to end."),
...
).WithDescription("Span is one span of a trace, flattened.")
...
var measurementSchema = schema.Struct[Measurement]("Measurement",
	...
	schema.FieldOf("operation", schema.Text(),
		...
		WithDescription("Operation is a declared name, or the bounded catch-all."),
	schema.FieldOf("status", schema.Text(),
	...
	schema.FieldOf("medianMicros", schema.Int64(),
		...
		WithDescription("MedianMicros is a bucket bound, not an interpolation: the bound at or below which half the measurements fell."),
	schema.FieldOf("p95Micros", schema.Int64(),
		...
		WithDescription("P95Micros is the bound at or below which nineteen measurements in twenty fell."),
	schema.FieldOf("p99Micros", schema.Int64(),
		...
		WithDescription("P99Micros is the bound at or below which ninety-nine measurements in a hundred fell."),
	...
	schema.FieldOf("delayMicros", schema.Int64(),
		func(value Measurement) int64 { return value.DelayMicros },
		func(value *Measurement, field int64) { value.DelayMicros = field }),
).WithDescription("Measurement is one bounded label's counts and timings.")
...
var routeSchema = schema.Struct[Route]("Route",
	...
	schema.FieldOf("status", schema.Int64(),
		...
		WithDescription("Status is what the endpoint answers with when it succeeds."),
).WithDescription("Route is one endpoint of the surface being served.")
...
var SnapshotSchema = schema.Struct[Snapshot]("Snapshot",
	...
	schema.FieldOf("time", schema.TextFormat("date-time"),
	...
	schema.FieldOf("costs", schema.List(costSchema),
		...
		WithDescription("Costs are what the process spent while each route's work ran, the most allocated first."),
	schema.FieldOf("fibers", schema.List(fiberSchema),
		...
		WithDescription("Fibers are the fibers running at this instant, nested as they were forked and flattened by depth."),
	schema.FieldOf("liveWork", liveWorkSchema,
		func(value Snapshot) LiveWork { return value.LiveWork },
		func(value *Snapshot, field LiveWork) { value.LiveWork = field }),
	schema.FieldOf("openSpans", schema.List(spanSchema),
		...
		WithDescription("OpenSpans are the spans running at this instant."),
	schema.FieldOf("trace", schema.List(spanSchema),
		...
		WithDescription("Trace is the recent events read as a tree, flattened."),
	schema.FieldOf("looseEvents", schema.Int64(),
		...
		WithDescription("LooseEvents belong to no span in the window: work outside every span, or a span that started before it."),
	...
	schema.FieldOf("drops", schema.Int64(),
...
).WithDescription("Attribute is one of a span's annotations, rendered.")
...
var fiberSchema = schema.Struct[Fiber]("Fiber",
	...
	schema.FieldOf("operation", schema.Text(),
		...
		WithDescription("Operation is what was running when the fiber was forked, which is the nearest thing to a name a fiber has."),
	...
	schema.FieldOf("ageMicros", schema.Int64(),
		...
		WithDescription("AgeMicros is how long it has been running, which is the first thing to know about a fiber that is still going."),
).WithDescription("Fiber is one fiber the runtime is running.")

var liveWorkSchema = schema.Struct[LiveWork]("LiveWork",
	schema.FieldOf("counted", schema.Bool(),
		func(value LiveWork) bool { return value.Counted },
		func(value *LiveWork, field bool) { value.Counted = field }).
		WithDescription("Counted says the runtime was built to keep these counters; without it the two numbers are zero because nobody is counting."),
	schema.FieldOf("fibers", schema.Int64(),
		func(value LiveWork) int64 { return value.Fibers },
		func(value *LiveWork, field int64) { value.Fibers = field }),
	schema.FieldOf("resources", schema.Int64(),
		func(value LiveWork) int64 { return value.Resources },
		func(value *LiveWork, field int64) { value.Resources = field }),
).WithDescription("Owned is what the runtime still holds.")
