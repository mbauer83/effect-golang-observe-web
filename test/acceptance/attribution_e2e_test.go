...
func TestEverySpanOfATraceCarriesTheSameIdentityAndTracesDiffer(t *testing.T) {
	...
	for _, span := range snapshot.Trace {
		...
		if known, seen := perRoot[root]; seen && known != span.Trace {
			t.Fatalf("expected one identity per trace, got %q and %q under %d",
				known, span.Trace, root)
		}
...
func TestAStagesRunIsAttributableToItsSpan(t *testing.T) {
	...
	for _, span := range snapshot.Trace {
		spanRuns, accounted := runs[span.Name]
		if !accounted {
			continue
		}
		within := 0
		for _, run := range spanRuns {
			if run.EndMicros >= span.StartMicros-1 &&
				run.EndMicros <= span.StartMicros+span.Micros+1 {
				within++
			}
		}
		if within != 1 {
